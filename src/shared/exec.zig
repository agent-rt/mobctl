...
/// `printer` 决定每行输出格式（见 logfmt.Printer）。`grep` 子串过滤后**实时**
/// 写出（每行 flush，follow 模式下持续到子进程退出或被信号杀掉——父进程被
/// Ctrl-C 时同进程组的子进程一起收到 SIGINT）。spawn 失败静默返回。
pub fn streamLogs(io: Io, argv: []const []const u8, w: *Io.Writer, grep: ?[]const u8, printer: *logfmt.Printer) void {
    ...
    if (child.stdout) |*so| {
        ...
        while (true) {
            ...
            printer.emit(w, line) catch break;
...
/// 共享同一个 printer。
pub fn writeFilteredText(w: *Io.Writer, text: []const u8, grep: ?[]const u8, printer: *logfmt.Printer) void {
    ...
    while (it.next()) |line| {
        ...
        printer.emit(w, line) catch return;
    }
}
